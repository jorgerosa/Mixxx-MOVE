...
void DotSceneLoader::processScene(TiXmlElement *XMLRoot)
{
	...
	if(XMLRoot->Attribute("ID")){ message += ", id " + String(XMLRoot->Attribute("ID")); };
	if(XMLRoot->Attribute("sceneManager")){ message += ", scene manager " + String(XMLRoot->Attribute("sceneManager")); };
	if(XMLRoot->Attribute("minOgreVersion")){ message += ", min. Ogre version " + String(XMLRoot->Attribute("minOgreVersion")); };
	if(XMLRoot->Attribute("author")){ message += ", author " + String(XMLRoot->Attribute("author")); };
	...
	/// if(pElement){ processEnvironment(pElement); }; // environment from ".scene" files --> Is turned OFF by me!

	// Process terrain (?)
	pElement = XMLRoot->FirstChildElement("terrain");
	if(pElement){ processTerrain(pElement); };
	...
	/// if(pElement){ processLight(pElement); }; // lights from ".scene" files --> Are turned OFF by me!

	// Process camera (?)
	pElement = XMLRoot->FirstChildElement("camera");
	/// if(pElement){ processCamera(pElement); }; // cameras from ".scene" files --> Are turned OFF by me!
};
...
void DotSceneLoader::processEnvironment(TiXmlElement *XMLNode)
{
	...
	if(pElement){ processFog(pElement); };

	// Process skyBox (?)
	pElement = XMLNode->FirstChildElement("skyBox");
	if(pElement){ processSkyBox(pElement); };

	// Process skyDome (?)
	pElement = XMLNode->FirstChildElement("skyDome");
	if(pElement){ processSkyDome(pElement); };

	// Process skyPlane (?)
	pElement = XMLNode->FirstChildElement("skyPlane");
	if(pElement){ processSkyPlane(pElement); };

	// Process clipping (?)
	pElement = XMLNode->FirstChildElement("clipping");
	if(pElement){ processClipping(pElement); };

	// Process colourAmbient (?)
	pElement = XMLNode->FirstChildElement("colourAmbient");
	if(pElement){ mSceneMgr->setAmbientLight(parseColour(pElement)); };
